...
function GameServer() {
    ...
    this.config = {
        ...
        serverWelcome1: 'Welcome to Private Server',      // First server welcome message
        ...
        defaultName: "Privat Server",      // Default name for all minions if name is not specified using command
...
GameServer.prototype.autoSplit = function (client, cell1) {
    ...
    if (checkSize && cell1._size > maxSize && cell1.getAge(this.tickCounter) >= 15) {
        if (client.cells.length >= !this.config.autoSplit) {
            ...
            splitMass = splitSize * splitSize / 50,
...
GameServer.prototype.resolveCollision = function (manifold) {
    ...
    // check distance
    var eatDistance = maxCell._size - minCell._size / 4;
    ...
    if (minCell.owner && minCell.owner == maxCell.owner) {
        ...
        if (minCell.getAge(this.tickCounter) < 10 || maxCell.getAge(this.tickCounter) < 10) {
            // just splited => ignore
            return;
        }
        ...
        // Size check
        if (maxCell._size < minCell._size * 1.10) {
            // too large => can't eat
            return;
        }
    }
...
GameServer.prototype.checkRigidCollision = function (c) {
    ...
    if (c.cell1.owner.mergeOverride)
        return false;
    if (c.cell1.getAge(this.tickCounter) < 10 || c.cell2.getAge(this.tickCounter) < 10) {
        // just splited => ignore
        return false;
    }
...
GameServer.prototype.pingServerTracker = function () {
    ...
    var obj = {
        ...
        version: 'Private Server ' + this.version,       // [optional]  server version
    ...
    var data = 'current_players=' + totalPlayers +
               ...
               '&version=Private Server ' + this.version +
               '&start_time=' + this.startTime;
    trackerRequest({
        host: 'ogar.mivabe.nl',
        port: 80,
